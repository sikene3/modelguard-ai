...
def run_local_monitoring(
    ...
    *,
    ...
    policy_hash = monitoring_config_hash(policy)
    ...
    verify_target_identity(metadata, spec.target_identity)
    known_non_targets: list[EventIdentity] = []
    for bundle_path in spec.known_non_target_bundle_paths:
        known_metadata = inspect_bundle(bundle_path)
        identity = target_identity_from_bundle(known_metadata)
        if identity == spec.target_identity or identity in known_non_targets:
    ...
    drift_evaluation = evaluate_drift(classified.accepted_events, metadata.baseline, policy)
    ...
    quality = assess_data_quality(
        ...
        minimum_accepted_events=policy.minimum_accepted_events,
    ...
    return MonitoringRunResult(
        report=report,
        published=published,
        monitoring_config_hash=policy_hash,
    )
